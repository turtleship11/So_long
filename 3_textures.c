...
/*   Updated: 2025/07/29 14:39:25 by jaeklee          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void load_textures(t_textures *textures)
{
    ...
    {
    	if (textures->wall) mlx_delete_texture(textures->wall);
    	if (textures->floor) mlx_delete_texture(textures->floor);
    	if (textures->player) mlx_delete_texture(textures->player);
    	if (textures->item) mlx_delete_texture(textures->item);
    	if (textures->exit) mlx_delete_texture(textures->exit);
...
void create_images(t_game *game, t_textures *textures)
{
	...
	game->exit_img = mlx_texture_to_image(game->mlx, textures->exit);

	if (!game->wall_img || !game->floor_img || !game->player_img || !game->item_img || !game->exit_img)
	{
